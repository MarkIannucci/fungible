...
import { syncAll, resetItemCursor } from '../../core/sync.js';
...
export const registry = {
  ...
  sync: {
    ...
    // Clear one item's cursor and replay its full Plaid history. Composed here
    // rather than in the renderer so the two steps can't be interleaved with
    // another sync, and merged so other institutions keep their failure badges.
    replayItemHistory: async (itemId: string) => {
      await resetItemCursor(itemId);
